...
<div class="body-row">
	<div>
		<div class="aside-sticky-nav">
			<a <?php echo $newsActive; ?> href="result">
				<div class="aside-nav-background tile-news"><?php get_t('title1_type_results', $bdd); ?></div>
			</a>
			<a <?php echo $sciencesActive; ?> href="result?type=sciences">
				<div class="aside-nav-background tile-sciences"><?php get_t('title2_type_results', $bdd); ?></div>
			</a>
			<a <?php echo $patentsActive; ?> href="result?type=patents">
				<div class="aside-nav-background tile-patents"><?php get_t('title3_type_results', $bdd); ?></div>
			</a>
			<form class="aside-nav-background tile-export selectExportType" method="post" action="result?page=<?php echo $data['page'] . $searchSort . $data['optionalCond'] . $data['orderBy'] . '&type=' . $type;?>" target="_blank" onsubmit="window.location.reload();">
				<input type="hidden" name="nonce" value="<?php echo $nonce; ?>"/>
				<input type="submit" class="submit-button-alpha" title="Export results" value=""/>
				<select name="export">
					<option value="csv">
						&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;CSV
					</option>
	...
	<div class="body">
		<form class="form-window" method="get" action="result">
		...
		<form class="body" method="post" action="result?page=<?php echo $data['page'] . $searchSort . $data['optionalCond'] . $data['orderBy'] . '&type=' . $type;?>">
			<input type="hidden" name="nonce" value="<?php echo $nonce; ?>"/>
				...
				<table>
					<thead>
						<tr>
							<th>
								<div>
									<input type="checkbox" name="selectAll" id="checkAllPage" />
									<label class="checkbox" for="checkAllPage" onmouseup="checkAllPage();"></label>
									<input title="<?php get_t('Delete selected links', $bdd); ?>" name="deleteLink" class="button-submit-trash" type="submit" value="" />
								</div>
							</th>
							<?php
							echo '
					...
					<tbody>
						...
						foreach (new LimitIterator($readOwnerResults, $base, $limit) as $result)
						{
							...
							<tr>
								...
								. $amIRead .
								'<td>
								<a href="addLinkInWiki?link">
									<img alt="'. var_get_t('Add in wiki', $bdd) . '" title="'. var_get_t('Add in wiki', $bdd) . '" class="wiki-logo" src="/images/iconWikiLight.png"/>
								</a>
								</td>
							</tr>';
						}
		...
		<div class="pages">
			...
			while ($cpt <= ($nbPage + 1))
			{
				...
				{
					echo '
					<a href="result?page=' . ($page - 5) . $searchSort . $data['optionalCond'] . $data['orderBy'] . '&type=' . $type . '" class="page-number speedPage">
					&lt;&lt; 5&nbsp;
					</a>';
				}
				...
				{
					echo '
					<a href="result?page=' . $cpt . $searchSort . $data['optionalCond'] . $data['orderBy'] . '&type=' . $type . '" class="page-number current">
					' . $cpt . '
					</a>';
					$dotBetweenPageNumber = FALSE;
				}
				...
				{
					echo '
					<a href="result?page=' . $cpt . $searchSort . $data['optionalCond'] . $data['orderBy'] . '&type=' . $type . '" class="page-number">
					' . $cpt . '
					</a>';
					$dotBetweenPageNumber = FALSE;
				}
