<?php
		...
		$req = $bdd->query('SELECT id, applicable_owners_sources FROM keyword_news_serge WHERE 1');
			...
			foreach ($applicable_owners_sources as $input_lines)
			{
				...
				$req = $bdd->prepare('UPDATE keyword_news_serge SET active = :active WHERE id = :id');
				$req->execute(array(
					'active' => $cpt,
					'id' => $input_lines['id']));
					$req->closeCursor();
