<?php
function addNewKeyword(string $sourceId, string $newKeyword, string $ERROR_MESSAGE, array $ownerSources, $bdd)
{
	$updateBDD = TRUE;

	// Check if keyword is already in bdd
	$checkCol = array(array('keyword', '=', mb_strtolower($newKeyword), ''));
	$result   = read('inquiries_news_serge', 'id, applicable_owners_sources, active', $checkCol, '', $bdd);
	$result   = $result[0] ?? '';
	...
	else
	{
		...
		if ($pos !== false)
		{
			...
			if (preg_match("/\|" . $_SESSION['id'] . ":[,0-9,]*," . $sourceId . ",[,0-9,]*\|/", $applicableOwners) && $sourceId != '0')
			{
				$updateBDD     = FALSE;
